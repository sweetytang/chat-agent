...
export default function ApprovalCard({ interrupt, onRespond, submitting = false }: ApprovalCardProps) {
    ...
    const summaryText = useMemo(() => {
        if (actions.length === 1) {
            return firstAction?.description ?? `Agent 请求执行操作: ${firstAction?.name ?? firstAction?.action}`;
        }
                    ...
                    {mode === "edit" ? (
                        <div className={styles.approvalEditHeader}>
                            <div className={styles.approvalActionTag}>
                                ...
                                <span className={styles.approvalActionName}>{action.name ?? action.action}</span>
                            </div>
