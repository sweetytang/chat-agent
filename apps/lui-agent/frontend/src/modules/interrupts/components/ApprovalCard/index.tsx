...
export function ApprovalCard({
  ...
  return (
    <div className={styles.card}>
      <strong>{isResolving ? '正在恢复运行…' : `工具需要审核：${tool}`}</strong>
      <div>
        <button disabled={isResolving} onClick={() => void resolve('approve')} type="button">
          批准
        </button>
        <button disabled={isResolving} onClick={() => void resolve('edit')} type="button">
          编辑
        </button>
        <button disabled={isResolving} onClick={() => void resolve('reject')} type="button">
          拒绝
        </button>
      </div>
    </div>
