...
export function Sidebar({ disabled = false }: { disabled?: boolean }) {
  ...
  async function createThread() {
    ...
    try {
      ...
      /* 登录失效时保留演示线程，仍可继续测试模型链路。 */
    }
  }
  useEffect(() => {
    if (token) void loadThreads();
  }, [token, loadThreads]);
  return (
    <aside className={styles.sidebar} aria-label="线程列表">
      ...
      {threads.length === 0 ? (
        <p className={styles.empty}>暂无已保存会话</p>
      ) : (
        threads.map((thread) => (
          <button
