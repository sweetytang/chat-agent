...
export function Sidebar({ disabled = false }: { disabled?: boolean }) {
  ...
  return <aside className={styles.sidebar} aria-label="线程列表">
    <div className={styles.heading}><h2>会话</h2><button disabled={disabled} onClick={() => void createThread()} type="button">新建</button></div>
    {threads.length === 0 ? <p className={styles.empty}>暂无已保存会话</p> : threads.map((thread) => <button className={`${styles.item} ${thread.id === threadId ? styles.active : ""}`} disabled={disabled} key={thread.id} onClick={() => setThread(thread.id, thread.title ?? "新对话", thread.current_checkpoint_id)} type="button">{thread.title ?? "未命名会话"}</button>)}
  </aside>;
}
