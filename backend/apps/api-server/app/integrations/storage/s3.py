...
class S3ObjectStorage:
    def __init__(self, *, bucket: str, client: Any = None, endpoint_url: str | None = None) -> None:
        if not bucket:
    ...
    async def put(self, data: bytes, metadata: ObjectMetadata) -> ObjectMetadata:
        self.client.put_object(
            Bucket=self.bucket, Key=metadata.key, Body=data, ContentType=metadata.content_type
        )
    ...
    async def presign(
        self, key: str, *, owner_id: str, run_id: str, tool_call_id: str, expires_seconds: int = 300
    ) -> str:
