...
async def list_stdio_definitions(user: User = Depends(current_user)) -> list[dict[str, str]]:
    ...
    commands = [
        item.strip() for item in get_settings().mcp_stdio_commands.split(",") if item.strip()
    ]
...
async def update_server(
    ...
    target_transport = cast(
        McpTransport,
        payload.transport if "transport" in payload.model_fields_set else server.transport,
    )
    if "transport" in payload.model_fields_set and target_transport is not server.transport:
        if target_transport is McpTransport.STDIO:
            ...
            if not payload.command:
                raise HTTPException(
                    status_code=422, detail="stdio MCP 必须提供管理员批准的 command"
                )
            allowed = {
                item.strip()
                for item in get_settings().mcp_stdio_commands.split(",")
                if item.strip()
            }
            if payload.command not in allowed:
                raise HTTPException(status_code=403, detail="stdio command 不在管理员预装清单中")
    ...
    if target_transport is McpTransport.STDIO and any(
        ...
        command = (
            ...
            else server.approved_config.get("command")
        )
        ...
        if "env" in payload.model_fields_set:
            approved["env"] = {key: value.strip() for key, value in (payload.env or {}).items()}
