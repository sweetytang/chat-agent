...
async def invoke_agent_driver(
    ...
    *,
    ...
    approval_tool_names = frozenset(
        ...
        if getattr(snapshot, "require_approval", True) and name not in thread.approved_tools
    )
    ...
    await RunRepository(session).update_status(UUID(run_id), RunStatus.COMPLETED)
