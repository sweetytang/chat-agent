...
def test_backend_install_syncs_all_workspace_packages() -> None:
    makefile = Path(__file__).parents[4] / "Makefile"
    source = makefile.read_text()

    assert "install-backend: ## 安装后端依赖" in source
    assert "cd $(BACKEND_DIR) && uv sync --locked --all-packages" in source
...
def test_mcp_preference_migration_deduplicates_and_adds_unique_constraints() -> None:
    migration = (
        Path(__file__).parents[1] / "migrations" / "versions" / "0005_mcp_preferences_unique.py"
    )
    source = migration.read_text()
